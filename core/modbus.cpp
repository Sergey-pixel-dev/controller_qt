...
int ModbusClient::ReadHoldingRegisters(uint16_t *buf,
                                       uint16_t startAdress,
                                       uint16_t NbReg,
                                       int timeout)
{
...
int ModbusClient::WriteHoldingRegisters(uint16_t *buf,
                                        uint16_t startAdress,
                                        uint16_t NbReg,
                                        int timeout)
{
    ...
    if (crc16(response->packageBuf, 6) != ((response->packageBuf[7] << 8) | response->packageBuf[6]))
        return -2;

    return NbReg;
}

int ModbusClient::ReadDiscrete(uint8_t *buf, uint16_t startAdress, uint16_t NbDis, int timeout)
{
    std::unique_ptr<Package<uint8_t>> pack1 = std::make_unique<Package<uint8_t>>();
    uint8_t *requestBuf = new uint8_t[8];
    pack1->packageBuf = requestBuf;
...
int ModbusClient::ReadCoils(uint8_t *buf, uint16_t startAdress, uint16_t NbCoils, int timeout)
{
    std::unique_ptr<Package<uint8_t>> pack1 = std::make_unique<Package<uint8_t>>();
    uint8_t *requestBuf = new uint8_t[8];
    pack1->packageBuf = requestBuf;
...
int ModbusClient::WriteCoils(uint8_t *buf, uint16_t startAdress, uint16_t NbCoils, int timeout)
{
